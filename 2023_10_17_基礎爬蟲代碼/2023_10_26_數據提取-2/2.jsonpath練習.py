...
# 等熟了再無腦用
# result = jsonpath.jsonpath(info, '$..title')[0]
# print(result)
...
# 8. 获取store中所有商品价格信息
result = jsonpath.jsonpath(info, '$..*.price')
print(result)
result = jsonpath.jsonpath(info, '$.store..price')
print(result)

# 9. 获取带有isbn的书
result = jsonpath.jsonpath(info, '$.store.book[?(@.isbn)].title')
print(result)
...
# 11. 获取价格在5-10这个区间的书籍
result = jsonpath.jsonpath(info, '$.store.book[?(@.price>=5 && @.price<=10)].title')
print(result)
